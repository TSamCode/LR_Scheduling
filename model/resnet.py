...
class ResNetSplit(nn.Module):
    ...
    '''
    A class to construct a ResNet18 model with two parallel branches.

    The code has been amended from the official PyTorch GitHub repository
    https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py
    (site last accessed at 13:48 on 6th September 2021)
    '''
...
def ResNetSplit18(num_classes=2):
    '''
    A function that creates a ResNet18 with two parallel branches,
    the two branches of this model have no parameters shared between them.
    The model has two nodes in the output layer
    '''
    return ResNetSplit(block=BasicBlock, layers=[2,2,2,2], num_classes=num_classes)


def ResNetSplit18Shared(num_classes=2):
    '''
    A function that creates a ResNet18 with two parallel branches,
    the two branches of this model shared parameters in the first two BasicBlock's
    The model has two nodes in the output layer
    '''
    return ResNetSplitShared(block=BasicBlock, layers=[2,2,2,2], num_classes=num_classes)